...
function display_practice_information() --Code to display practice information
	...
	if DontDisplaySock then
		gui.text(0, 16, "S:0000-0", text_back_colour, text_back_colour)
		gui.text(0, 16, "S:", text_colour, "clear")
	else
		gui.text(0, 16, string.format("S:%d%03X-%d", xstring, sock, ypos), text_colour, text_back_colour)
	end
	...
	gui.text(0, 24, string.format(" :%03d", Frame), text_colour, text_back_colour)
	if ppu.readbyte(0x3F0D) == 7 then
		gui.text(0, 24, "F", "#7D0800", "clear")
	elseif ppu.readbyte(0x3F0D) == 0x17 then
		gui.text(0, 24, "F", "#CB4D0C", "clear")
	else
		gui.text(0, 24, "F", "#FF9A38", "clear")
	end
	
	gui.text(51, 8, string.format("A:%03d", memory.readbyte(User_Var_A)), text_colour, text_back_colour)
	
	gui.text(51, 16, string.format("B:%03d", memory.readbyte(User_Var_B)), text_colour, text_back_colour)
	...
	and memory.readbyte(ram_DisableScreenFlag) == 0 and WZ_or_Title_Remainder then
		...
		gui.text(51, 24, string.format("R:%02d", RemainderDisplay), text_colour, text_back_colour)
	else
		RemainderDisplay = -1
	end
	...
	gui.text(0, 8, string.format("R:%04d-%s", Rule, ScreenEnterDisplay), text_colour, text_back_colour)
end
...
function display_information()
	...
	gui.text(85, 8, string.format("XP:%02X.%X", memory.readbyte(ram_SprObject_X_Position), bit.rshift(memory.readbyte(ram_SprObject_X_MoveForce), 4)), text_colour, text_back_colour)
	gui.text(85, 16, string.format("YP:%02X.%02X", memory.readbyte(ram_SprObject_Y_Position), memory.readbyte(ram_SprObject_YMF_Dummy)), text_colour, text_back_colour)
	...
	if memory.readbytesigned(ram_Player_X_Speed) > -1 then
		gui.text(133, 8, string.format("XS:%d.%02X", memory.readbyte(ram_Player_X_Speed), memory.readbyte(ram_Player_X_MoveForce)), text_colour, text_back_colour)
	else
		gui.text(133, 8, string.format("XS:%d.%02X", memory.readbytesigned(ram_Player_X_Speed), AND(256 - memory.readbyte(ram_Player_X_MoveForce), 0xFF)), text_colour, text_back_colour)
	end
	if memory.readbytesigned(ram_Player_Y_Speed) > -1 then
		gui.text(133, 16, string.format("YS:%d.%02X", memory.readbyte(ram_Player_Y_Speed), memory.readbyte(ram_Player_Y_MoveForce)), text_colour, text_back_colour)
	else
		gui.text(133, 16, string.format("YS:%d.%02X", memory.readbytesigned(ram_Player_Y_Speed), AND(256 - memory.readbyte(ram_Player_Y_MoveForce), 0xFF)), text_colour, text_back_colour)
	end
	
	gui.text(187, 8, string.format("XA:%d.%02X", memory.readbyte(ram_FrictionAdderLow - 1), memory.readbyte(ram_FrictionAdderLow)), text_colour, text_back_colour) --Display X acceleration
	gui.text(187, 16, string.format("YA:0.%02X", memory.readbyte(ram_VerticalForce)), text_colour, text_back_colour) --Display Y acceleration
