...
function display_practice_information() --Code to display practice information
	...
	if DontDisplaySock then
		gui.text(0, 16, "S:    ", text_colour, text_back_colour)
	else
	...
	gui.text(64, 8, string.format("A:%03d", User_Var_ADisplay), text_colour, text_back_colour)
	
	gui.text(64, 16, string.format("B:%03d", User_Var_BDisplay), text_colour, text_back_colour)
	...
	or (memory.readbyte(ram_ScreenRoutineTask) == 7 or memory.readbyte(ram_ScreenRoutineTask) == 8) and memory.readbyte(ram_DisableScreenFlag) == 0 then
		...
		gui.text(64, 24, string.format("R:%02d", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	else
		StarFlagTaskControlDisplay = -1
	end
	...
	if memory.readbyte(ram_DisableScreenFlag) ~= 0 or memory.readbyte(ram_GameEngineSubroutine) == 0 then
		gui.text(0, 8, "R:    ", text_colour, text_back_colour)
	else
...
function display_information()
	...
	if memory.readbytesigned(ram_Player_X_Speed) > -1 then
		...
		gui.text(137, 8, string.format("XS:%d.%02X", memory.readbytesigned(ram_Player_X_Speed), AND(256 - memory.readbyte(ram_Player_X_MoveForce), 0xFF)), text_colour, text_back_colour)
	end
	if memory.readbytesigned(ram_Player_Y_Speed) > -1 then
		...
		gui.text(137, 16, string.format("YS:%d.%02X", memory.readbytesigned(ram_Player_Y_Speed), AND(256 - memory.readbyte(ram_Player_Y_MoveForce), 0xFF)), text_colour, text_back_colour)
	end
