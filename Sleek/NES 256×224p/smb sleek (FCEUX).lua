--This lua script is supposed to be identical to the smb3.lua script (https://github.com/fortenbt/smb3-lua) but for SMB, for FCEUX, and a 256×224p sleek version
...
function display_remainder(x, mod) --Function to display the remainder and to compact the code to display Pellsson information
	...
	if StarFlagTaskControlDisplay < 10 then
		gui.text(64, 24, string.format("R:%d ", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	else
		gui.text(64, 24, string.format("R:%d", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	end
end
...
function display_pellsson() --Code to display Pellsson information
	...
	gui.text(0, 16, string.format("S:%04X", sock), text_colour, text_back_colour)
	...
	gui.text(35, 8, string.format(" :%s", ScreenEnterDisplay), text_colour, text_back_colour)
	gui.pixel(35, 10, text_colour)
	gui.pixel(36, 11, text_colour)
	gui.pixel(37, 12, text_colour)
	gui.pixel(38, 13, text_colour)
	gui.pixel(39, 14, text_colour)
	gui.pixel(39, 10, text_colour)
	gui.pixel(38, 11, text_colour)
	gui.pixel(36, 13, text_colour)
	gui.pixel(35, 14, text_colour)
	...
	gui.text(35, 16, string.format(" :%03d", Frame), text_colour, text_back_colour)
	if ppu.readbyte(0x3F0D) == 7 then
		gui.text(35, 16, "F", "#7D0800", "clear")
	elseif ppu.readbyte(0x3F0D) == 0x17 then
		gui.text(35, 16, "F", "#CB4D0C", "clear")
	else
		gui.text(35, 16, "F", "#FF9A38", "clear")
	end
	...
	gui.text(64, 8, string.format("X:%03d", XOrgDisplay), text_colour, text_back_colour)
	
	gui.text(64, 16, string.format("Y:%03d", YOrgDisplay), text_colour, text_back_colour)
	...
	elseif memory.readbyte(ram_GameEngineSubroutine) == 3 then
		...
		StarFlagTaskControlDisplay = -1
		gui.text(64, 24, "R:  ", text_colour, text_back_colour)
	end
	...
	gui.text(0, 8, string.format("R:%04d", Rule), text_colour, text_back_colour)
end

function display_spriteslots()
	local y_counter = 33 --for listing sprites and removing blank spriteslot's spaces
	for i = 1, 6, 1 do
		if memory.readbyte(ram_Enemy_Flag + i - 1) ~= (toggle_display_sprite_information_after_death and -1 or 0) then --if the sprite isn't dead, unless ..._after_death is set
			if memory.readbyte(ram_Enemy_Flag + i - 1) == 0 then --If dead, display faded text and background
				gui.text(0, y_counter, string.format("%d:%02X", i - 1, memory.readbyte(ram_Enemy_ID + i - 1)), text_faded_colour, text_faded_back_colour) --display sprite slot number and sprite ID
				gui.text(23, y_counter, string.format("(%02X.%X, %02X.%02X)", memory.readbyte(ram_SprObject_X_Position + i), bit.rshift(memory.readbyte(ram_SprObject_X_MoveForce + i), 4), memory.readbyte(ram_SprObject_Y_Position + i), memory.readbyte(ram_SprObject_YMF_Dummy + i)), text_faded_colour, text_faded_back_colour) --draw position
				...
				gui.text(0, y_counter, string.format("%d:%02X", i - 1, memory.readbyte(ram_Enemy_ID + i - 1)), text_colour, text_back_colour) --display sprite slot number and sprite ID
				gui.text(23, y_counter, string.format("(%02X.%X, %02X.%02X)", memory.readbyte(ram_SprObject_X_Position + i), bit.rshift(memory.readbyte(ram_SprObject_X_MoveForce + i), 4), memory.readbyte(ram_SprObject_Y_Position + i), memory.readbyte(ram_SprObject_YMF_Dummy + i)), text_colour, text_back_colour) --draw position
				y_counter = y_counter + 8 --add to y_counter so the next sprite is shown below the previous
			end
		end
	end
...
function display_information()
	if memory.readbyte(ram_IntervalTimerControl) < 10 then --Done to make the display look nice
		gui.text(230, 8, string.format("FR: %d", memory.readbyte(ram_IntervalTimerControl)), text_colour, text_back_colour)
	else
		gui.text(230, 8, string.format("FR:%d", memory.readbyte(ram_IntervalTimerControl)), text_colour, text_back_colour)
	end
	
	--display mario information
	gui.text(93, 8, string.format("XP:%02X.%X", memory.readbyte(ram_SprObject_X_Position), bit.rshift(memory.readbyte(ram_SprObject_X_MoveForce), 4)), text_colour, text_back_colour)
	gui.text(93, 16, string.format("YP:%02X.%02X", memory.readbyte(ram_SprObject_Y_Position), memory.readbyte(ram_SprObject_YMF_Dummy)), text_colour, text_back_colour)
	...
	if memory.readbytesigned(ram_Player_X_Speed) > -1 then
		gui.text(137, 8, string.format("XS:%d.%02X", memory.readbyte(ram_Player_X_Speed), memory.readbyte(ram_Player_X_MoveForce)), text_colour, text_back_colour)
	else
		gui.text(137, 8, string.format("XS:%d.%02X", memory.readbytesigned(ram_Player_X_Speed), (256 - memory.readbyte(ram_Player_X_MoveForce)) % 256), text_colour, text_back_colour)
	end
	if memory.readbytesigned(ram_Player_Y_Speed) > -1 then
		gui.text(137, 16, string.format("YS:%d.%02X", memory.readbyte(ram_Player_Y_Speed), memory.readbyte(ram_Player_Y_MoveForce)), text_colour, text_back_colour)
	else
		gui.text(137, 16, string.format("YS:%d.%02X", memory.readbytesigned(ram_Player_Y_Speed), (256 - memory.readbyte(ram_Player_Y_MoveForce)) % 256), text_colour, text_back_colour)
	end
	
	gui.text(186, 8, string.format("XSA:%d.%02X", memory.readbyte(ram_FrictionAdderLow - 1), memory.readbyte(ram_FrictionAdderLow)), text_colour, text_back_colour) --Display X SpeedAdder
	gui.text(186, 16, string.format("YSA:0.%02X", memory.readbyte(ram_VerticalForce)), text_colour, text_back_colour) --Display Y SpeedAdder
		
	gui.text(230, 16, string.format("ST?:%d", memory.readbyte(ram_Player_State)), text_colour, text_back_colour)
end
