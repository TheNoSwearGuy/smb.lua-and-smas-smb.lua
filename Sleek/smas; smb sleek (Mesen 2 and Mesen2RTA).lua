...
function display_practice_information() --Code to display practice information
	...
	local xstringvalue = (((emu.read(wram_SprObject_PageLoc, emu.memType.snesMemory) << 12)
		...
		+ (emu.read(wram_SprObject_X_MoveForce, emu.memType.snesMemory) >> 4)) % x) >> 3
	...
	for i = 0, 9, 1 do
		if emu.read(wram_FloateyNum_Timer + i, emu.memType.snesMemory) == 0x2A then
			EnemyFrame = true
			break
		end
	end
	...
	elseif BowserFrame then
		for i = 0, 8, 1 do
			if emu.read(wram_Enemy_Flag + i, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + i, emu.memType.snesMemory) == 0x2D
			and emu.read(wram_SprObject_X_Position + i + 1, emu.memType.snesMemory) ~= emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
	...
	drawString(51, 7, string.format("A:%03d", emu.read(User_Var_A, emu.memType.snesMemory)), text_colour, text_back_colour)
	
	drawString(51, 15, string.format("B:%03d", emu.read(User_Var_B, emu.memType.snesMemory)), text_colour, text_back_colour)
	...
	if emu.read(0x7FFF00, emu.memType.snesMemory) == 2 then --If playing SMAS: SMB1
	...
	elseif emu.read(0x7FFF00, emu.memType.snesMemory) == 4 then --If playing SMAS: SMB2J
		BackwardsPole = emu.read(wram_PlayerFacingDir, emu.memType.snesMemory) == 2
	end
	...
	for i = 1, 10, 1 do
		if emu.read(wram_Enemy_Flag + i - 1, emu.memType.snesMemory) ~= (toggle_display_sprite_information_after_death and -1 or 0) then --if the sprite isn't dead, unless ..._after_death is set
			if emu.read(wram_Enemy_Flag + i - 1, emu.memType.snesMemory) == 0 then --If dead, display faded text and background
				...
				drawString(23, y_counter, string.format("(%02X.%X, %02X.%02X)", emu.read(wram_SprObject_X_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_X_MoveForce + i, emu.memType.snesMemory) >> 4, emu.read(wram_SprObject_Y_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_YMF_Dummy + i, emu.memType.snesMemory)), text_faded_colour, text_faded_back_colour) --draw position
				...
				drawString(23, y_counter, string.format("(%02X.%X, %02X.%02X)", emu.read(wram_SprObject_X_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_X_MoveForce + i, emu.memType.snesMemory) >> 4, emu.read(wram_SprObject_Y_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_YMF_Dummy + i, emu.memType.snesMemory)), text_colour, text_back_colour) --draw position
				y_counter = y_counter + 8 --add to y_counter so the next sprite is shown below the previous
			end
		end
	end
...
function display_time()
	...
	if emu.read(0xFF4, emu.memType.snesMemory) & 0x10 == 0 and emu.read(0xE67, emu.memType.snesMemory) == 0 then
		start_pressed = false
	end
	if start_pressed and emu.read(0xE67, emu.memType.snesMemory) > 0 then
		start_reached = true
	end
...
function display_information()
	...
	for i = 0, 4, 1 do
		if emu.read(wram_Enemy_ID + i, emu.memType.snesMemory) == 0x2D then
			DisplayBowserHP = true
			break
		end
	end
	if DisplayBowserHP then --Only display Bowser HP when a Bowser has been loaded
		drawString(236, 23, string.format("HP:%d", emu.read(wram_BowserHitPoints, emu.memType.snesMemory)), text_colour, text_back_colour)
	end
	
	--display mario information
	drawString(85, 7, string.format("XP:%02X.%X", emu.read(wram_SprObject_X_Position, emu.memType.snesMemory), emu.read(wram_SprObject_X_MoveForce, emu.memType.snesMemory) >> 4), text_colour, text_back_colour)
	drawString(85, 15, string.format("YP:%02X.%02X", emu.read(wram_SprObject_Y_Position, emu.memType.snesMemory), emu.read(wram_SprObject_YMF_Dummy, emu.memType.snesMemory)), text_colour, text_back_colour)
	...
	if emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1) > -1 then
		drawString(133, 7, string.format("XS:%d.%02X", emu.read(wram_Player_X_Speed, emu.memType.snesMemory), emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory)), text_colour, text_back_colour)
	else
		drawString(133, 7, string.format("XS:%d.%02X", emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory)) & 0xFF), text_colour, text_back_colour)
	end
	if emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1) > -1 then
		drawString(133, 15, string.format("YS:%d.%02X", emu.read(wram_Player_Y_Speed, emu.memType.snesMemory), emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)), text_colour, text_back_colour)
	else
		drawString(133, 15, string.format("YS:%d.%02X", emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)) & 0xFF), text_colour, text_back_colour)
	end
	
	drawString(187, 7, string.format("XA:%d.%02X", emu.read(wram_FrictionAdderLow - 1, emu.memType.snesMemory), emu.read(wram_FrictionAdderLow, emu.memType.snesMemory)), text_colour, text_back_colour) --Display X acceleration
	drawString(187, 15, string.format("YA:0.%02X", emu.read(wram_VerticalForce, emu.memType.snesMemory)), text_colour, text_back_colour) --Display Y acceleration
	
	drawString(230, 15, string.format("ST?:%d", emu.read(wram_Player_State, emu.memType.snesMemory)), text_colour, text_back_colour)
end

function calculations()
	if emu.read(0x7FFF00, emu.memType.snesMemory) == 2 or emu.read(0x7FFF00, emu.memType.snesMemory) == 4 then --If playing SMAS: SMB1 or SMAS: SMB2J
