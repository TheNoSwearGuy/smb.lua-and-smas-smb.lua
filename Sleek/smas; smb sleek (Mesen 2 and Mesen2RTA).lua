...
function display_practice_information() --Code to display practice information
	...
	if emu.read(wram_IntervalTimerControl, emu.memType.snesMemory) & 3 == 2 then
		sock = (sockvalue & 0xFFF) + (emu.read(wram_SprObject_Y_Position, emu.memType.snesMemory) & 3) * 0x1000
		DontDisplaySock = false
	end
	...
	if DontDisplaySock then
		emu.drawString(0, 15, "S:    ", text_colour, text_back_colour)
	else
	...
	if emu.read(wram_Enemy_Flag, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID, emu.memType.snesMemory) == 0x2D
	and emu.read(wram_SprObject_X_Position + 1, emu.memType.snesMemory) == emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
	or emu.read(wram_Enemy_Flag + 1, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + 1, emu.memType.snesMemory) == 0x2D
	and emu.read(wram_SprObject_X_Position + 2, emu.memType.snesMemory) == emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
	or emu.read(wram_Enemy_Flag + 2, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + 2, emu.memType.snesMemory) == 0x2D
	and emu.read(wram_SprObject_X_Position + 3, emu.memType.snesMemory) == emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
	or emu.read(wram_Enemy_Flag + 3, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + 3, emu.memType.snesMemory) == 0x2D
	and emu.read(wram_SprObject_X_Position + 4, emu.memType.snesMemory) == emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
	or emu.read(wram_Enemy_Flag + 4, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + 4, emu.memType.snesMemory) == 0x2D
	and emu.read(wram_SprObject_X_Position + 5, emu.memType.snesMemory) == emu.read(wram_BowserOrigXPos, emu.memType.snesMemory) then
		BowserFrame = true
	end
	...
	or emu.read(wram_Sample7SoundQueue, emu.memType.snesMemory) == 6 then
		BowserFrame = false
	...
	elseif BowserFrame then
		...
		and emu.read(wram_SprObject_X_Position + 4, emu.memType.snesMemory) ~= emu.read(wram_BowserOrigXPos, emu.memType.snesMemory)
		or emu.read(wram_Enemy_Flag + 4, emu.memType.snesMemory, 1) > 0 and emu.read(wram_Enemy_ID + 4, emu.memType.snesMemory) == 0x2D
		and emu.read(wram_SprObject_X_Position + 5, emu.memType.snesMemory) ~= emu.read(wram_BowserOrigXPos, emu.memType.snesMemory))
	...
	User_Var_ADisplay = emu.read(User_Var_A, emu.memType.snesMemory)
	User_Var_BDisplay = emu.read(User_Var_B, emu.memType.snesMemory)
	...
	or emu.read(wram_ScreenRoutineTask, emu.memType.snesMemory) >= 7 and emu.read(wram_ScreenRoutineTask, emu.memType.snesMemory) <= 9 and emu.read(wram_DisableScreenFlag, emu.memType.snesMemory) == 0 then
		if StarFlagTaskControlDisplay == -1 then
			Frame = emu.read(wram_FrameCounter, emu.memType.snesMemory)
			StarFlagTaskControlDisplay = emu.read(wram_IntervalTimerControl, emu.memType.snesMemory)
		end
...
local function hitbox(x1, y1, x2, y2) --Function to draw the hitboxes
	if emu.read(wram_FrameCounter, emu.memType.snesMemory) & 1 == 0 or toggle_display_hitbox_collision_check == false then
...
function display_information()
	...
	if emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1) > -1 then
		...
		drawString(137, 7, string.format("XS:%d.%02X", emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory)) & 0xFF), text_colour, text_back_colour)
	end
	if emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1) > -1 then
		...
		drawString(137, 15, string.format("YS:%d.%02X", emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)) & 0xFF), text_colour, text_back_colour)
	end
