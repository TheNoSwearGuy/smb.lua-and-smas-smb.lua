--This lua script is supposed to be identical to the smb3.lua script (https://github.com/fortenbt/smb3-lua) but for SMB, for FCEUX, and a 256×240p sleek version
...
function display_remainder(x, mod) --Function to display the remainder and to compact the code to display Pellsson information
	...
	if StarFlagTaskControlDisplay < 10 then
		gui.text(64, 16, string.format("R:%d ", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	else
		gui.text(64, 16, string.format("R:%d", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	end
end
...
function display_pellsson() --Code to display Pellsson information
	...
	gui.text(64, 0, string.format("X:%03d", XOrgDisplay), text_colour, text_back_colour)
	
	gui.text(64, 8, string.format("Y:%03d", YOrgDisplay), text_colour, text_back_colour)
	...
	elseif memory.readbyte(ram_GameEngineSubroutine) == 3 then
		...
		StarFlagTaskControlDisplay = -1
		gui.text(64, 16, "R:  ", text_colour, text_back_colour)
	end
...
function display_time()
	...
	if negative_delay then --If negative delay, show negative time before timing starts
		if emu.framecount() - start_frame < 0 then
			gui.text(188, 232, string.format("-%02d:%02d:%02d.%03d", hours, minutes, seconds, milliseconds), text_colour, text_back_colour) --draw it
		else
			gui.text(193, 232, string.format("%02d:%02d:%02d.%03d", hours, minutes, seconds, milliseconds), text_colour, text_back_colour) --draw it
		end
	else --Otherwise, show 0 hours, 0 minutes, 0 seconds, and 0 milliseconds until timing starts
		if emu.framecount() - start_frame < 0 then
			gui.text(193, 232, "00:00:00.000", text_colour, text_back_colour) --draw 0 hours, 0 minutes, 0 seconds, and 0 milliseconds
		else
			gui.text(193, 232, string.format("%02d:%02d:%02d.%03d", hours, minutes, seconds, milliseconds), text_colour, text_back_colour) --draw it
		end
	end
...
function display_information()
	if memory.readbyte(ram_IntervalTimerControl) < 10 then --Done to make the display look nice
		gui.text(230, 0, string.format("FR: %d", memory.readbyte(ram_IntervalTimerControl)), text_colour, text_back_colour)
	else
		gui.text(230, 0, string.format("FR:%d", memory.readbyte(ram_IntervalTimerControl)), text_colour, text_back_colour)
	end
	...
	if memory.readbytesigned(ram_Player_X_Speed) > -1 then
		...
		gui.text(137, 0, string.format("XS:%d.%02X", memory.readbytesigned(ram_Player_X_Speed), (256 - memory.readbyte(ram_Player_X_MoveForce)) % 256), text_colour, text_back_colour)
	end
	if memory.readbytesigned(ram_Player_Y_Speed) > -1 then
		...
		gui.text(137, 8, string.format("YS:%d.%02X", memory.readbytesigned(ram_Player_Y_Speed), (256 - memory.readbyte(ram_Player_Y_MoveForce)) % 256), text_colour, text_back_colour)
	end
