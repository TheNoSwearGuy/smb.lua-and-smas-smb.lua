--This lua script is supposed to be identical to the smb3.lua script (https://github.com/fortenbt/smb3-lua) but for SMAS: SMB and for Mesen 2
...
function display_remainder(x) --Function to display the remainder and to compact the code to display Pellsson information
	...
	if StarFlagTaskControlDisplay < 10 then
		drawString(1, 62, string.format("R:%d ", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	else
		drawString(1, 62, string.format("R:%d", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	end
end
...
function display_pellsson() --Code to display Pellsson information
	...
	drawString(1, 38, string.format(" rame:%03d", Frame), text_colour, text_back_colour)
	...
	drawString(1, 46, string.format("X:%03d", XOrgDisplay), text_colour, text_back_colour)
	
	drawString(1, 54, string.format("Y:%03d", YOrgDisplay), text_colour, text_back_colour)
	...
	elseif emu.read(wram_GameEngineSubroutine, emu.memType.snesMemory) == 3 then
		...
		StarFlagTaskControlDisplay = -1
		drawString(1, 62, "R:  ", text_colour, text_back_colour)
	end
	...
	if BackwardsPole then
		emu.drawString(1, 78, "Pole?: Yes", text_colour, 0xFF000000)
	else
		emu.drawString(1, 78, "Pole?: No", text_colour, 0xFF000000)
	end
end
	...
	for i = 1, 10, 1 do
		if emu.read(wram_Enemy_Flag + i - 1, emu.memType.snesMemory) ~= (toggle_display_sprite_information_after_death and -1 or 0) then --if the sprite isn't dead, unless ..._after_death is set
			if emu.read(wram_Enemy_Flag + i - 1, emu.memType.snesMemory) == 0 then --If dead, display faded text and background
				...
				emu.drawString(23, y_counter, string.format("(%02X.%X, %02X.%02X)", emu.read(wram_SprObject_X_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_X_MoveForce + i, emu.memType.snesMemory) >> 4, emu.read(wram_SprObject_Y_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_YMF_Dummy + i, emu.memType.snesMemory)), text_faded_colour, text_faded_back_colour) --draw position
				...
				emu.drawString(23, y_counter, string.format("(%02X.%X, %02X.%02X)", emu.read(wram_SprObject_X_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_X_MoveForce + i, emu.memType.snesMemory) >> 4, emu.read(wram_SprObject_Y_Position + i, emu.memType.snesMemory), emu.read(wram_SprObject_YMF_Dummy + i, emu.memType.snesMemory)), text_colour, text_back_colour) --draw position
				y_counter = y_counter + 8 --add to y_counter so the next sprite is shown below the previous
			end
		end
	end
...
function display_information()
	...
	if toggle_display_mario_velocity then --Display X Speed, the CORRECT X SubSpeed value, Y Speed, and the CORRECT Y SubSpeed value
		...
		elseif emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1) > -1 and emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1) < 0 then
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.snesMemory), emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory), emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)) % 256), text_colour, text_back_colour)
		elseif emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1) < 0 and emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1) > -1 then
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory)) % 256, emu.read(wram_Player_Y_Speed, emu.memType.snesMemory), emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)), text_colour, text_back_colour)
		else
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.snesMemory)) % 256, emu.read(wram_Player_Y_Speed, emu.memType.snesMemory, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.snesMemory)) % 256), text_colour, text_back_colour)
		end
		y_counter = y_counter + 8
	end
	...
	if toggle_display_mario_velocity_adder then
		...
		drawString(1, y_counter + 8, string.format("Adder: (%d.%02X, 0.%02X)", emu.read(wram_FrictionAdderLow - 1, emu.memType.snesMemory), emu.read(wram_FrictionAdderLow, emu.memType.snesMemory), emu.read(wram_VerticalForce, emu.memType.snesMemory)), text_colour, text_back_colour) --Display "Adder:" and SpeedAdder
