...
--Note: the "Backwards Pole?" feature for SMAS: SMB1 isn't 100% accurate, but it's 98.8% accurate. For SMAS: SMB2J, it's 100% accurate.
...
function display_practice_information() --Code to display practice information
	...
	if DontDisplaySock then
		drawString(1, 9, "S:0000-0", text_back_colour, text_back_colour)
		emu.drawString(1, 9, "S:", text_colour, 0xFF000000)
	else
		drawString(1, 9, string.format("S:%d%03X-%d", xstring, sock, ypos), text_colour, text_back_colour)
	end
	...
	drawString(34, 1, string.format("-%s", ScreenEnterDisplay), text_colour, text_back_colour)
	...
	drawString(1, 17, string.format(" :%03d", Frame), text_colour, text_back_colour)
	if emu.read(0xB, emu.memType.cgram) == 3 and emu.read(0xA, emu.memType.cgram) == 0x5F then
		drawString(1, 17, "F", 0xFFD600, 0xFF000000)
	elseif emu.read(0xB, emu.memType.cgram) == 3 and emu.read(0xA, emu.memType.cgram) == 0xFF then
		drawString(1, 17, "F", 0xFFFF00, 0xFF000000)
	else
		drawString(1, 17, "F", 0xFFFFFF, 0xFF000000)
	end
	
	drawString(1, 25, string.format("A:%03d", emu.read(User_Var_A, emu.memType.cpu)), text_colour, text_back_colour)
	
	drawString(1, 33, string.format("B:%03d", emu.read(User_Var_B, emu.memType.cpu)), text_colour, text_back_colour)
	...
	and emu.read(wram_DisableScreenFlag, emu.memType.cpu) == 0 and WZ_or_Title_Remainder then
		...
		drawString(1, 41, string.format("R:%02d", RemainderDisplay), text_colour, text_back_colour)
	else
		RemainderDisplay = -1
	end
	...
	--Display the result
	emu.drawRectangle(0, 48, 50, 9, text_back_colour, text_back_colour)
	emu.drawString(1, 49, "Backwards", text_colour, 0xFF000000)
	emu.drawRectangle(0, 56, 50, 9, text_back_colour, text_back_colour)
	emu.drawString(1, 57, "Pole?: "..(BackwardsPole and "Yes" or "No"), text_colour, 0xFF000000)
end
...
function display_information()
	...
	if toggle_display_mario_velocity then --Display X Speed, the CORRECT X SubSpeed value, Y Speed, and the CORRECT Y SubSpeed value
		...
		elseif emu.read(wram_Player_X_Speed, emu.memType.cpu, 1) > -1 and emu.read(wram_Player_Y_Speed, emu.memType.cpu, 1) < 0 then
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.cpu), emu.read(wram_Player_X_MoveForce, emu.memType.cpu), emu.read(wram_Player_Y_Speed, emu.memType.cpu, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.cpu)) & 0xFF), text_colour, text_back_colour)
		elseif emu.read(wram_Player_X_Speed, emu.memType.cpu, 1) < 0 and emu.read(wram_Player_Y_Speed, emu.memType.cpu, 1) > -1 then
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.cpu, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.cpu)) & 0xFF, emu.read(wram_Player_Y_Speed, emu.memType.cpu), emu.read(wram_Player_Y_MoveForce, emu.memType.cpu)), text_colour, text_back_colour)
		else
			drawString(1, y_counter, string.format("Speed: (%d.%02X, %d.%02X)", emu.read(wram_Player_X_Speed, emu.memType.cpu, 1), (256 - emu.read(wram_Player_X_MoveForce, emu.memType.cpu)) & 0xFF, emu.read(wram_Player_Y_Speed, emu.memType.cpu, 1), (256 - emu.read(wram_Player_Y_MoveForce, emu.memType.cpu)) & 0xFF), text_colour, text_back_colour)
		end
		y_counter = y_counter + 8
	end
