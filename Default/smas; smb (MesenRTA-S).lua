...
function display_practice_information() --Code to display practice information
	...
	if DontDisplaySock then
		drawString(1, 1, "S:    ", text_colour, text_back_colour)
	else
		drawString(1, 1, string.format("S:%04X", sock), text_colour, text_back_colour)
	end
	...
	drawString(1, 31, string.format(" :%03d", Frame), text_colour, text_back_colour)
	...
	drawString(1, 39, string.format("A:%03d", User_Var_ADisplay), text_colour, text_back_colour)
	
	drawString(1, 47, string.format("B:%03d", User_Var_BDisplay), text_colour, text_back_colour)
	...
	or emu.read(wram_ScreenRoutineTask, emu.memType.cpu) >= 7 and emu.read(wram_ScreenRoutineTask, emu.memType.cpu) <= 9 and emu.read(wram_DisableScreenFlag, emu.memType.cpu) == 0 then
		...
		drawString(1, 55, string.format("R:%02d", StarFlagTaskControlDisplay), text_colour, text_back_colour)
	else
		StarFlagTaskControlDisplay = -1
	end
	...
	emu.drawString(1, 71, "Pole?: "..(BackwardsPole and "Yes" or "No"), text_colour, 0xFF000000)
end
